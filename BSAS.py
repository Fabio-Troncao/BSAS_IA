...
def load_data(f_name, destination='',  delimiter=','):
    data = np.genfromtxt("D:\Documentos\aulas  EAD UFT\IA\BSAS\datasets-uci-iris.csv", delimiter=delimiter)  # Initialize array with data
...
def cluster_vectors(f_name, theta_min, theta_max, theta_step, out_file, destination='',  delimiter=',', q=-1, precision=2):
    ...
    np.savetxt("D:\Documentos\aulas  EAD UFT\IA\BSAS\datasets-uci-iris.csv", output, delimiter=delimiter, fmt='%.'+str(precision)+'f')